...
def draw_image(slider_value):
    # calculate coordinates of points on circle and store in lists
    x_list = []; y_list = []
    for n in range(m.get()):
        angle = 2.0 * pi * n / m.get()  + pi / 2.0
        x_list.append( x_center - int(radius * cos(angle)) )
        y_list.append( y_center - int(radius * sin(angle)) )
    ...
    txt = f"Modular multiplication circle, m = {m.get()}, p = {p.get()}"
...
radius = 450
line_color = "white"
default_file_name = "mod_mult_circle.png"
...
root1.title("Modular multiplication circle using Python and tkinter, right click to save")
m = tk.IntVar()
m.set(m_max)
p = tk.IntVar()
p.set(159)
